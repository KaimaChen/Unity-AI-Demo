...
public class DStarLite : LPAStar
{
    ...
    private bool MoveOneStep()
    {
        ...
        {
            #region show
            m_curtStart.SetSearchType(SearchType.Path, true);
        ...
        {
            Debug.LogError($"生成路径失败，在{m_curtStart.Pos}处中断");
            return false;
        }
    ...
    protected void CheckNearChanged()
    {
        ...
        //假设检测器只能检查附近的点
        List<SearchNode> neighbors = GetNeighbors(m_curtStart);
        for(int i = 0; i < neighbors.Count; i++)
        {
